...
function ChatUser({ user, handleListItemClick, activeUserId }) {
    ...
    onSnapshot(orderedColl, (querySnapshot) => {
      ...
      querySnapshot.forEach((doc) => {
        messages.push({
          ...doc.data(),
          id: doc.id,
          createdAt: convertTimestamp(doc.data().createdAt, "userFormat"),
        });
      });
      const message = messages.filter(
        (item) => item.uid === user.uid && item.name !== "Chuck"
      );
  ...
  return (
    <li
      ...
      className={
        activeUserId === user.uid
          ? "chats__item chats__item--active"
          : "chats__item"
      }
    ...
    >
      ...
      <time className="message-date" dateTime={lastMessageDate}>
        {lastMessageDate}
      </time>
    </li>
  );
