...
import { onSnapshot, collection, addDoc, serverTimestamp, query, orderBy, doc, updateDoc } from 'firebase/firestore';
import getAnswer from '../../api/answer';
import db from '../../api/api';
import { setMessages, setShowChatWindowOnly, setCurrentUserId } from '../../store/chatSlice'
...
function ChatWindow() {
  ...
  const messages = useSelector((state) => state.chat.messages).filter((item) => item.uid === activeUserId);
  ...
  useEffect(() => {
    ...
    onSnapshot(orderedColl, (querySnapshot) => {
      ...
      querySnapshot.forEach((doc) => {
        messages.push({...doc.data(), id: doc.id, createdAt: convertTimestamp(doc.data().createdAt, 'messageFormat')});
      });
  ...
  return (
    <div className="chat" style={isMobileView ? {width: '100vw'} : {width: '70vw'}}>
      <header className="chat__header">
        <MobileView style={{marginTop: '5px', marginLeft: '10px'}}>
          <button className="chat__header-btn-back" onClick={handleMobileViewClick} />
        </MobileView>
        {activeUser && <img className="chat__header-avatar" src={activeUser.avatarUrl} alt="author" width="50" height="50" />}
        <span className="chat__header-name">{activeUser && activeUser.name}</span>
      </header>
      ...
      <footer className="chat__input-wrapper">
        <textarea ref={messageRef} onKeyDown={handleEnterKeyDown} className="chat__input" placeholder="Type your message" />
        <img onClick={handleSendMessageClick} className="send-message-btn" src="images/send-icon.svg" alt="send message" width="35" height="35"/>
      </footer>
    </div>
  );
}
